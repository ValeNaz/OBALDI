...
const SellerDashboard: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    setSubmitted(true);
    // In produzione invio a /api/seller/products
  };
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            <div className="grid md:grid-cols-2 gap-6">
              <div>
                ...
                <input required type="text" className="w-full p-3 border rounded-lg focus:ring-2 focus:ring-[#0b224e] outline-none" placeholder="Esempio: Token hardware" />
              </div>
              <div>
                <label className="block text-sm font-bold text-slate-700 mb-2">Prezzo (€)</label>
                <input required type="number" step="0.01" className="w-full p-3 border rounded-lg focus:ring-2 focus:ring-[#0b224e] outline-none" placeholder="0.00" />
              </div>
            </div>
            <div>

...
const ProductDetail: React.FC = () => {
  ...
  const product = {
    ...
    description: 'Il Kit Protezione Web V1 è stato progettato per chi lavora in ambienti digitali ad alto rischio. Include un firewall hardware pre-configurato e un manuale operativo per la mitigazione dei rischi di phishing e injection. Ogni componente è stato testato dal nostro team tecnico.',
    fullSpecs: [
        ...
        <div className="flex flex-col">
          ...
          <div className="bg-slate-50 p-6 rounded-xl border mb-8">
            ...
            <ul className="space-y-2 text-sm text-slate-600">
              {product.fullSpecs.map((s, i) => <li key={i} className="flex items-center"><span className="w-1.5 h-1.5 bg-[#a41f2e] rounded-full mr-3" /> {s}</li>)}
            </ul>
          </div>
          ...
          {user?.isMember ? (
            ...
            <div className="bg-slate-100 p-6 rounded-xl text-center">
              <p className="text-sm font-medium text-slate-600 mb-4">L'acquisto di questo prodotto è riservato ai membri Obaldi.</p>
              <Link to="/membership" className="inline-block py-3 px-8 bg-[#a41f2e] text-white font-bold rounded-lg">
                Diventa membro ora
              </Link>
            </div>
          )}
